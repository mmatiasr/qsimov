...
def make_imports():
    ...
    global load_dataset, make_split, load_model, get_optimizer
    global build_path_selector_vgg16_linear
    ...
    from experiments.cifar10_gradient_by_splits.keras_model_factory import (
        load_model,
        get_optimizer,
        build_path_selector_vgg16_linear,
    )
    from experiments.cifar10_gradient_by_splits.preprocess_data import (
        load_dataset,
        make_split,
...
def save_results(name, model_name, model_type, model, history, results_dir):
    ...
    model.save(
        f"{results_dir}/{name}{model_type}_{model_name}_model.tf",
        save_format="tf",
    )
...
def _save_linear_clone(softmax_model, results_dir, split_name, model_name):
    """Save a linear-output clone of the trained path selector (same weights).

    The clone shares identical convolutional and dense weights with the softmax
    model. The only difference is the final activation (linear vs. softmax).
    This clone is required by QsimovLinearSystem which rejects softmax outputs.
    """
    linear_model = build_path_selector_vgg16_linear()
    for src_layer, dst_layer in zip(softmax_model.layers, linear_model.layers):
        src_weights = src_layer.get_weights()
        if src_weights:
            dst_layer.set_weights(src_weights)

    save_path = (
        f"{results_dir}/{split_name}_path_selector_linear_{model_name}_model.tf"
    )
    linear_model.save(save_path, save_format="tf")
...
def execute_logic(results_dir, split, args):
    ...
    model = load_model(args.model_name, model_type)

    history = train_model(
        model, split_train_x, split_train_y, test_x, test_y, args
    )
    ...
    # When training the path selector, also save a linear clone for LinearSystem
    if args.train_path_selector and args.model_name == "vgg16":
        _save_linear_clone(model, results_dir, split_to_name(split), args.model_name)
