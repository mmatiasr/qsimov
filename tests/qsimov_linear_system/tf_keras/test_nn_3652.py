...
from tests.nn_mocks_keras import make_data_3652, make_model_3652
from tests.utils_load_save_tests import (
    assert_keras_qsimov_linear_system_equals,
)
import os
import tempfile
...
def make_qsimov_linear_full():
    ...
    return KerasQsimovLinearSystem(
        path_selector=KerasPathSelector(neural_network=model, initial_layer=0),
...
def make_qsimov_linear_partial():
    ...
    return KerasQsimovLinearSystem(
        path_selector=KerasPathSelector(
            neural_network=model, initial_layer=-2
        ),
        absolute_cutoff=1e-8,
    )
...
def test_save_load(qsimov_linear_full, data):
    for fit in (False, True):
        ...
        # load model
        qsimov_linear_full_loaded = KerasQsimovLinearSystem.load(path)
